	...
	try:
		...
		if req.status_code == 200:
			...
			for report in reports:
				if 'url' in report.keys() and 'bounty_disclosed' in report.keys() and 'formatted_bounty' in report.keys():
					if 'url' in report.keys():
						title = report['title']
						reportid = report['id']
						report_url = Base_url + report['url']
						print report_url
						bounty = report['formatted_bounty']
						# writing public reports to Mongo DB
						if db.reports.count({'reportid': reportid}) == 0:
							db.reports.insert({'reportid': reportid, 'title': title, 'report_url': report_url, 'bounty': bounty})
